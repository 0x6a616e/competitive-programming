...
// Damos nuestra palabra de que hemos realizado esta actividad con integridad
// académica Problema 2 Useless Counting
...
// Función recursiva que me dice de cuantas formas puedo escribir números que
// sumen n con l digitos
unsigned long long solve(int n, int l) {
    ...
    if (calculos[n][l] == -1) {
        ...
        if (n > 9 * l) {
            calculos[n][l] = 0;
            // Otro caso base, aquí llegaría cuando n sea entre 0 y 9 y l valga
            // 1, así que solo hay una forma de expresarlo
        } else if (l == 1) {
            calculos[n][l] = 1;
        } else {
    ...
    // Se regresa lo que esta memorizado, ya sea que se calculo ahorita o ya
    // estaba
    return calculos[n][l];
}
